                                         ...
                                         long inFlags, long *outFlags)
{
  ...
  if (inFlags & movieImportWithIdle) {
    ...
    *outFlags |= movieImportResultNeedIdles;
    ...
    AddVideoSampleRefsToQTMedia(store, segmentDuration);
    AddAudioSampleRefsToQTMedia(store, segmentDuration);
...
// given.
//
static OSErr AddSampleRefsToQTMedia(Media media, Track track,
  ...
  media_start = lround((double) times[0] / ns_per_sec * media_timescale);

  for (long i = 0; i < num_samples; ++i) {
    sample_duration = lround((double) times[i + 1] / ns_per_sec *
                             media_timescale) -
        lround((double) times[i] / ns_per_sec * media_timescale);
    samples[i].durationPerSample = sample_duration;
    media_duration += sample_duration;
  }

  if (last_time > 0) {
    sample_duration = lround((double) last_time / ns_per_sec *
                             media_timescale) -
        lround((double) times[num_samples] / ns_per_sec * media_timescale);
