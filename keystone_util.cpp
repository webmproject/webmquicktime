...
std::string ReadHomeDirectoryPath() {
  ...
  if (passwd_buf_size > 0) {
    char* passwd_buf = new char[passwd_buf_size];
    if (passwd_buf) {
...
void TouchActivityFile() {
  ...
  // Set accessed/modified time for |file_desc| to now. Not bothering with the
  // return value: caller does not care... at least we tried?
  utime(activity_file.c_str(), NULL);
  close(file_desc);
}
